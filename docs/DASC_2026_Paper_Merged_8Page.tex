...
the UAV case, the tool reveals a single-string internal-bus weakness and 22
excess privileges that persist across all strategies.
\end{abstract}
...
Worst amplification& N/A    & 2.105$\times$ & N/A \\
Runtime (s)        & 12.228 & 71.054 & 12.482 \\
\bottomrule
\end{tabular}
\end{table}
...
reveals 22 excess privileges and a narrow internal-bus trust boundary. Under
\textit{min\_resources}, the architecture exposes a 2.105$\times$ worst-case
amplification under policy-server compromise. In other words, even when
feature assignment looks strong, the bus organization can still dominate the
security story.
...
or runtime adaptation. Those are important next steps, but they do not reduce
the value of the current design-time workflow.
...
showed that even when high-security assignments drive residual risk to zero,
the architecture can still retain serious privilege and trust-boundary
weaknesses.
