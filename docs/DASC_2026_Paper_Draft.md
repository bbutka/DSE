...
Modern avionics increasingly consolidate safety-critical and security-sensitive functions onto shared System-on-Chip (SoC) platforms. This hardware consolidation creates new attack surfaces where a compromised IP core can reach safety-critical assets through shared bus interconnects. We present an automated design space exploration (DSE) workflow that synthesizes zero-trust security architectures for avionics SoCs. Using Answer Set Programming (ASP), the tool takes a single architectural model and produces four deliverables: (1) Pareto-optimal security feature assignments under FPGA resource constraints, (2) least-privilege access control policies with lifecycle mode awareness, (3) resilience checklists derived from automated fault and compromise scenarios, and (4) multi-strategy comparison reports. The workflow employs an additive risk model aligned with NIST SP 800-30 that separately budgets security risk and availability risk, enabling independent optimization of feature selection and redundancy architecture. We evaluate the approach on two case studies: an 8-component SoC drone and an 11-component UAV translated from the DARPA CASE surveillance architecture. The tool identifies 9 excess privileges, 6 trust anchor gaps, and a 3.0x risk reduction from zero-trust overlay on the SoC drone, while revealing single-string architecture vulnerabilities in the UAV that manual hardening approaches did not surface.
...
This paper presents an automated DSE workflow that addresses these challenges through Answer Set Programming (ASP). Given a single SoC architectural model, the tool produces four deliverables:
...
4. **Strategy comparison reports** — multi-dimensional scoring across security, resources, power, latency, and resilience for three optimization strategies (maximum security, minimum footprint, balanced).
...
- Evaluation on two case studies demonstrating generality across SoC architectures.
...
### B. Threat Model
We consider an attacker who can compromise any single component or bus, with effects propagating through the interconnect topology. The model captures 13 threat classes including:
...
where DomainBonus maps trust domains to [0,3], ExploitMod = exploitability − 3 ∈ [−2, +2], EffProtect dispatches to security (read/write) or availability (avail) protection tables, and LogProtect is the logging feature's detection capability.

**Dual-Risk Budget.** Two independent hard constraints replace a single risk cap:
...
where P_combined is the normalized product of individual failure probabilities across group members.
...
**Firewall and policy server placement** with cost minimization:
```prolog
...
**Least-privilege analysis** comparing topology-implied access against declared needs:
```prolog
excess_privilege(M, C, Op) :- granted_op(M, C, Op), not access_need(M, C, Op).
```
...
The tool is implemented in Python with the Clingo 5.8 ASP solver. The codebase comprises:
- **15 ASP encoding files** (3,476 lines): Phase 1 optimization, Phase 2 ZTA synthesis, Phase 3 resilience analysis, and runtime monitoring
- **IP catalog**: Xilinx Zynq-7000 post-implementation resource estimates from Vivado for all security/logging features on the PYNQ-Z2 target (xc7z020clg400-1)
- **GUI**: Network topology editor, progress monitoring, results panel with attack paths, capability assessment, and comparative analysis
- **Pipeline orchestrator**: Runs three strategies (max security, min footprint, balanced) across all three phases, ranks solutions, and generates reports

The tool supports two input modes: (1) built-in factory models (SoCDrone, RefSoC-16) loadable from the GUI, and (2) custom topologies defined through the network editor with drag-and-drop component placement.
...
**Phase 1 Results.** The solver achieved proven optimality in <5 seconds, assigning security features that minimize total risk to 51 while consuming only 17% of available LUTs (9,340/53,200) and 1% of power budget (184mW/15W). The assignments range from zero_trust+zero_trust_logger on c6 (high-impact, no latency constraint) to mac+no_logging on the redundancy group members (where probabilistic risk reduction from 5-way redundancy dominates). The safety-critical c8 receives zero_trust+some_logging despite having the tightest latency constraint (5 cycles for read), demonstrating the solver's ability to navigate the security-latency trade-off.

**Table II: TC9 Phase 1 — Optimal Security Assignments**

| Component | Domain | Security | Logging | Risk | Constraint |
|---|---|---|---|---|---|
| c1 | high | dynamic_mac | zero_trust_logger | 12 | — |
| c2 | high | mac | zero_trust_logger | 12 | — |
| c3 | high | mac | no_logging | 7 | — |
| c4 | high | mac | some_logging | 10 | — |
| c5 | high | mac | zero_trust_logger | 10 | — |
| c6 | high | zero_trust | zero_trust_logger | 0 | — |
| c7 | low | dynamic_mac | no_logging | 0 | — |
| c8 | high | zero_trust | some_logging | 0 | latency=5cy |
...
Key findings: (1) The ZTA overlay reduces worst-case risk by 3.0x compared to no-ZTA baseline (153.0 → 51.0). (2) ps0 compromise (2.50x) is the highest-impact single event because ps0 governs both firewalls; its compromise bypasses all policy enforcement. (3) The redundancy group provides availability resilience (3-of-5 quorum) but NOT security independence: noc0 failure cuts off all 5 members simultaneously (common-mode failure). (4) c8's 5-cycle latency constraint is the dominant source of residual risk — relaxing it to 12 cycles would reduce c8r1 write risk by 33%.
...
| Component | Domain | Security | Logging | Exploit |
|---|---|---|---|---|
| radio_drv | low | zero_trust | zero_trust_logger | 5 (RF) |
| fpln | normal | zero_trust | zero_trust_logger | 3 |
| wpm | normal | zero_trust | zero_trust_logger | 3 |
| cam_mgr | normal | dynamic_mac | zero_trust_logger | 3 |
| wifi_drv | untrusted | dynamic_mac | zero_trust_logger | 5 (WiFi) |
| uart_drv | normal | zero_trust | zero_trust_logger | 3 |
| nfzdb | high | zero_trust | zero_trust_logger | 2 |
| attest_gate | root | zero_trust | zero_trust_logger | 1 (CakeML) |
| geofence | high | zero_trust | zero_trust_logger | 1 (CakeML) |
| fpln_filt | high | zero_trust | zero_trust_logger | 1 (CakeML) |
| swu | privileged | dynamic_mac | zero_trust_logger | 4 |

**Phase 1 Results (min_resources strategy, best-found in 60 s).** LUT usage drops to 18% (9,870 LUTs, 190 mW) at the cost of total risk rising to 35. High-impact untrusted components retain stronger features (wifi_drv keeps zero_trust; radio_drv gets mac+some_logging), while verified components (attest_gate, geofence, fpln_filt) step down to dynamic_mac with minimal logging. This demonstrates the solver navigating the security-resource Pareto frontier: the 47% LUT reduction comes with 35 units of residual risk concentrated in availability assets.
...
We presented an automated DSE workflow for zero-trust avionics SoC security that produces Pareto-optimal architectures, access control policies, resilience checklists, and strategy comparisons from a single architectural model. The additive risk model with dual-risk budget enables principled separation of security and availability optimization. Evaluation on two case studies — an SoC drone and a DARPA CASE UAV translated from AADL — demonstrates the tool's ability to identify excess privileges, trust anchor gaps, and single-point-of-failure vulnerabilities that manual approaches miss, while proving optimality under FPGA resource constraints.

Future work includes multi-chip topology support, integration with DO-326A certification evidence generation, and extension to runtime adaptation where the tool's three-mode policies drive hardware reconfiguration in response to detected anomalies.
...
[2] Xilinx, "Zynq-7000 Isolation Design Flow," UG1085, 2022.

[3] S. Drzevitzky et al., "Secure on-chip interconnects for SoC platforms," *Proc. DATE*, 2019.

[4] RTCA, "DO-326A: Airworthiness Security Process Specification," 2014.

[5] ARINC, "ARINC 653: Avionics Application Software Standard Interface Part 1," 2015.

[6] S. Rose, O. Borchert, S. Mitchell, and S. Connelly, "Zero Trust Architecture," NIST SP 800-207, Aug. 2020.

[7] MITRE, "Implementing Zero Trust at the FAA," *MITRE Technical Report*, 2023.

[8] M. Hasan, I. Amundson, and D. Hardin, "Zero Trust Architecture Design Patterns for Cyber-Physical Systems," *SAE AeroTech*, 2023.

[9] M. Hasan, I. Amundson, and D. Hardin, "Assurance of Zero Trust Mechanisms for Cyber-Physical Systems," *J. Systems Architecture*, vol. 148, 2024.

[10] D. Hardin et al., "BriefCASE: Cyber Assured Systems Engineering," *DARPA CASE Program*, 2021.

[11] "Zero Trust Avionics Systems (ZTAS)," *Proc. IEEE/AIAA 42nd DASC*, pp. 1–8, 2023.

[12] FIRST, "Common Vulnerability Scoring System v3.1 Specification," 2019.

[13] NIST, "Guide for Conducting Risk Assessments," SP 800-30 Rev. 1, 2012.

[14] T. Aven, "On the meaning of a black, green, yellow or red risk in a risk matrix," *Reliability Eng. & System Safety*, vol. 154, pp. 143–148, 2017.

[15] M. Gebser, R. Kaminski, B. Kaufmann, and T. Schaub, "Answer Set Solving in Practice," *Synthesis Lectures on AI and ML*, Morgan & Claypool, 2012.
