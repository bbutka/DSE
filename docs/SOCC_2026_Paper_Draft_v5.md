...
System-on-Chip (SoC) platforms integrate processors, accelerators, memories, and peripherals on shared interconnects, making security architecture design a constrained optimization problem rather than a simple add-on decision. Security features, logging mechanisms, and enforcement points improve protection, but they also consume area, power, and latency budget. We present a design space exploration workflow for SoC security architecture synthesis based on Answer Set Programming (ASP). Given an architectural model, the method jointly selects per-component security and logging features and synthesizes enforcement placement under hard hardware constraints. The formulation uses an additive residual-risk model for standalone components, a closed-form normalized compromise model for redundant groups, and explicit resource, power, and latency constraints derived from the implementation catalog. It therefore supports exact optimization when solving completes and best-found fallback under a time limit when it does not. We evaluate the method on two representative SoC topologies and compare multiple optimization strategies, including maximum-security, minimum-resource, and balanced configurations. A bounded-incumbent continuation run proves the DARPA/UAV `min_resources` point at 8,410 LUTs and residual risk 39, while a 36-instance synthetic scaling sweep shows exact solutions on all small instances, mixed exact/best-found behavior on medium ones, and a clear tractability boundary between medium and large families. The results show that the solver identifies feasible high-security architectures within modest FPGA utilization, exposes topology-driven over-privilege and trust gaps, and makes tradeoffs among protection strength, implementation cost, and bottleneck constraints explicit. The workflow demonstrates that ASP-based DSE is a practical approach for hardware-aware SoC security architecture synthesis.
...
This case is useful because its three strategy runs expose a strong footprint-versus-risk tradeoff and a continuation-sensitive minimum-resource search.
...
| Phase 1 runtime (s) | 12.015 | 3788.100 | 12.271 |
| Phase 2 runtime (s) | 0.011 | 0.009 | 0.009 |
| Phase 3 runtime (s) | 0.202 | 1.000 | 0.202 |
| Total runtime (s) | 12.228 | 3789.409 | 12.482 |
| Phase 1 optimality proven | True | True | True |

The original `min_resources` point was timeout-limited, but a bounded-incumbent continuation run ultimately proved the optimum. Table IV-B shows the convergence trajectory across six attempts with exponentially increasing timeouts.

**Table IV-B. DARPA/UAV min\_resources convergence trajectory**

| Attempt | Timeout | Models explored | Best risk | Best LUTs | Power (mW) | Status |
|---:|---:|---:|---:|---:|---:|---|
...
| 5 | 1,200 s | 0 | 39 | 8,410 | 160 | no improvement |
| 6 | 1,800 s | 0 | 39 | 8,410 | 160 | **proven optimal** |

The solver explored 37 candidate models in the first four attempts, converging to the optimal point by attempt 4. Attempts 5 and 6 explored no additional models, confirming that the incumbent is globally optimal. The final design uses 8,410 LUTs, 6,160 FFs, 7 BRAMs, and 160 mW at residual risk 39. The resulting assignment is almost the cheapest feasible pattern: `mac` on every component, `some_logging` only where the hard constraints require it, and `no_logging` where the caps allow it.

Substantively, the DARPA/UAV results still support the strategy-front argument. Moving from `max_security` to `min_resources` reduces LUT use from 18,170 to 8,410 but raises residual risk from 0 to 39. The `balanced` strategy recovers zero residual risk while staying below the `max_security` footprint.
...
This case therefore complements TC9 in a different way. Its key weakness is not redundancy/common-mode interaction, but a single-string internal-bus organization plus a narrow trust boundary at the ingress path. The worst measured `min_resources` scenarios are `ps_uart_compromise` and `pep_mc_bypass`, both at 87.0 total risk or 2.062x baseline.

### C. Runtime and Synthetic Scaling View

To make the runtime claim explicit, Table V summarizes the measured end-to-end times for the main case-study strategy points.
...
| DARPA/UAV | min_resources | 11 receivers | 24 | 3789.409 | proven after continuation |
| DARPA/UAV | balanced | 11 receivers | 24 | 12.482 | proven |

These measured runtimes show that the method is practical at the reported problem scale, but that some strategy/objective combinations can become substantially harder than others. The DARPA `min_resources` point is the clearest example: it is exact, but only after incumbent-bounded continuation tightened the search over six attempts.

The synthetic sweep adds a broader tractability view than the two detailed case studies alone.

**Table VI. Synthetic scaling sweep summary**

| Size family | Runs | Solved | Exact | Best-found | Failed |
|---|---:|---:|---:|---:|---:|
| `S` | 9 | 9 | 9 | 0 | 0 |
| `M` | 9 | 9 | 3 | 6 | 0 |
| `L` | 9 | 1 | 0 | 1 | 8 |
...
The sweep reveals a clear transition. All small instances solved exactly across all three strategies. All medium instances produced valid fronts, but only `max_security` remained exact across the family; `min_resources` and `balanced` were best-found under the timeout budget. At large scale, only one `min_resources` bus-dominated instance produced a solution, and no extra-large instances solved. The current encoding is therefore effective through the medium family but needs stronger pruning, decomposition, or alternative formulations for larger synthetic SoCs.
...
The current results support five concrete claims. First, the method can produce exact strategy points for both detailed case studies, including the previously difficult DARPA/UAV `min_resources` point once incumbent-bounded continuation is allowed to finish. Second, the primary TC9 case exhibits a true three-strategy tradeoff rather than a single nominal optimum. Third, the measured runtimes are practical at the reported hand-built problem scale, though objective choice matters sharply. Fourth, topology-driven findings remain stable across strategy modes, which suggests that policy placement and privilege structure matter independently of feature tuning. Fifth, the synthetic sweep now provides a real tractability boundary: exact on all `S`, mixed exact/best-found on `M`, and largely unsolved beyond that.
...
The evaluation is still not broad enough to claim mature large-scale deployability. The synthetic sweep now demonstrates scaling behavior across 36 generated instances, but it also shows that the current encoding loses traction rapidly between the `M` and `L` families. The paper therefore supports a stronger methods claim than before, but not a claim of robust scalability to large SoCs under all objectives.
...
- large-family synthetic instances often return best-found or no-model outcomes under the current timeout budget
...
Measured evaluation on TC9 and the DARPA/UAV translation showed that the approach is practical at useful scale, exposes topology-driven weaknesses that local hardening alone would miss, and produces interpretable strategy fronts across security and footprint objectives. The DARPA/UAV continuation run further proved that the minimum-resource point is an 8,410-LUT design with residual risk 39, rather than merely a timeout-limited approximation. The synthetic sweep then showed exact results on all small instances, mixed exact/best-found behavior on medium ones, and a clear tractability transition before the large family. The strongest defensible claim is therefore that SoC security architecture design can be treated as a constrained synthesis problem with exact solutions when the search completes, explicit best-found fallbacks when it does not, and a now-measured scaling boundary for the current encoding. That is a materially stronger methods contribution and a credible basis for journal submission.
