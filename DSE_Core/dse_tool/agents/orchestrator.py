...
Phase 1 uses ILP by default in this package, with the legacy ASP/Clingo
solver retained as an automatic fallback when the ILP backend is
unavailable or returns an unsatisfiable result.
...
from .ilp_phase1_agent import ILPPhase1Agent
...
class DSEOrchestrator:
    ...
    def _run_phase1(self, strategy: str, instance_facts: str) -> Phase1Result:
        ...
        phase1_backend = (self.solver_config.get("phase1_backend") or "ilp").lower()
        ...
        p1 = self._run_phase1_ilp(strategy)
        if p1.satisfiable:
            return p1
        ...
        if fallback_backend == "asp":
            self._post(
                "WARNING",
                f"[Orchestrator] ILP Phase 1 unavailable/UNSAT for {strategy} - falling back to ASP.",
            )
    ...
    def _run_phase1_ilp(self, strategy: str) -> Phase1Result:
        p1_agent = ILPPhase1Agent(
