...
strong evidence of formulation correctness. An integer linear programming
(ILP) formulation solved by Gurobi confirms the same optima in under
0.003\,s, confirming the optima from a third solver.
At 20~components, ASP does not converge to
the proven optimum within a 120\,s timeout, while CP-SAT proves optimality in
under 0.1\,s, revealing a scalability gap that motivates a hybrid workflow. The benchmark demonstrates constraint sensitivity at
the 20-component scale: latency constraints alone force three high-impact
...
correct, and (iii)~evaluation on an architecture-derived benchmark
...
\item Cross-solver validation: equivalent CP-SAT and ILP models produce
identical proven-optimal objectives on all tested instances, providing
strong evidence of formulation correctness across three solver
paradigms (ASP, CP-SAT, ILP).
...
cross-solver validation of the reported optima, or (iii)~evaluate on a benchmark derived
from an open-source SoC at a scale beyond small synthetic cases.
...
explored in prior work~\cite{butka_measuring_2025}. LUT and FF counts
...
follows the multiplicative model of~\cite{butka_measuring_2025}:
\begin{equation}
...
As a third validation point, we encode the same problem as an
integer linear program and solve it with Gurobi~\cite{gurobi}.
Binary indicator variables select one feature pair per component;
resource constraints are linear sums; redundancy-group risk is
precomputed via a lookup table over feasible member combinations.
This formulation is less extensible than ASP but yields
an optimality certificate from a mature commercial solver.
...
cases and all three solvers) and the OpenTitan-derived case (OT).
...
with 32\,GB RAM, Windows~11, Python~3.12, clingo~5.8.0,
OR-Tools~9.x, and Gurobi~13.0. All solvers were run single-threaded
for fair comparison.
...
clear scalability gap between 8 and 20~components.
...
\bottomrule
\multicolumn{7}{l}{\scriptsize $^\dagger$Separate extended run; ASP suboptimal at timeout. Objectives are CP-SAT proven optima.} \\
\multicolumn{7}{l}{\scriptsize ILP (Gurobi) confirms the same objectives on all OT and most synthetic instances ($<$0.003\,s).}
\end{tabular}}
\end{table}
...
$220\times$ speedup. In a separate extended run at 20~components
(OpenTitan), ASP did not prove optimality after $>$225\,s, while
CP-SAT proves optimality in under 0.1\,s ($>$4{,}500$\times$ gap).
This scalability boundary marks where CP-SAT becomes essential.
An ILP formulation (Gurobi) confirms all optima in under 0.003\,s,
roughly $15\times$ faster than CP-SAT on the OT case.
...
\bottomrule
\multicolumn{4}{l}{\scriptsize $^\dagger$Separate extended run; ASP did not prove optimality.}
\end{tabular}
\end{table}
...
where ASP fails to prove optimality (separate extended run). For rapid parameter
...
\begin{figure}[t]
    ...
    \caption{Mean solve time vs.\ component count (log scale).
    At 20~components ASP cannot prove optimality ($>$225\,s,
    separate extended run).}
    \label{fig:runtime}
\end{figure}
...
weaker feature.  This multi-constraint coordination, where latency,
power, and redundancy structure interact simultaneously, is where
exact optimization adds the most value.
...
(transitive closures over degraded topologies,
bounded-depth attack-path enumeration). Both stages rely on
recursive reachability, default negation, and set/count
aggregation---patterns natural in ASP but requiring substantial
reformulation for CP-SAT or ILP.  We originally chose ASP for Phase~1
because it shares the solver with Phases~2 and~3; the cross-solver
validation reported here showed that Phase~1's constrained-assignment
structure is better served by CP-SAT or ILP, motivating a hybrid
architecture: CP-SAT or ILP for feature synthesis, ASP for the
downstream stages whose logic-programming structure resists
linearisation.

\subsection{Relation to ILP/MIP Solvers}

The resource constraints are linear and the objective can be
linearised, so ILP solvers can solve the same problem. We confirmed
this with Gurobi~\cite{gurobi}, which proves optimality on all OT
profiles in under 0.003\,s, roughly $15\times$ faster than CP-SAT.
We retain CP-SAT as the primary production solver because it is
open-source and its element constraints require less formulation
effort than the ILP lookup tables needed for redundancy groups.
The ILP comparison confirms that the CP-SAT optima are correct.
...
(Figure~\ref{fig:pareto}) gives a complete risk--resource tradeoff
...
measured vulnerability data, so the reported optima are optimal
under the chosen scoring heuristic, not under a calibrated security
metric.  Our feature catalog comes from Vivado synthesis of
standalone security IP cores on xc7z020; costs are transferred to
XC7K410T by primitive count, which is reasonable for LUT/FF totals
but does not account for placement, routing, or interconnect
effects that a post-implementation flow would capture.  Power values
are pre-route static estimates, not post-place-and-route
measurements.  The formulation covers the feature-synthesis stage;
the policy and resilience stages are implemented in the maintained
repository but not evaluated here.
Scaling beyond 20~components to 50--100 component SoCs remains
future work.
...
Gurobi ILP confirms the OT optima in under 0.003\,s.
Looking ahead, we plan to co-optimize firewall and policy placement
with feature selection and evaluate resilience under component-compromise
scenarios on the OpenTitan topology.
