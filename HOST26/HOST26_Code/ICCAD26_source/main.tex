...
security assignment exploiting redundancy-group structure that
greedy and local-search heuristics miss by 32\%.
\end{abstract}
...
consume the same LUT, FF, and power budget needed by the
functional design.
...
structure, producing a 32\% improvement over greedy and local-search
heuristics.
\end{enumerate}
...
On the solver side, we note that ASP has been used for hardware
...
\multicolumn{10}{l}{\scriptsize $^a$Power ($10^{-2}$\,W): same three-tier model as LUTs (byAsset + byComponent + base).}
\end{tabular}}
\end{table}
...
objective mismatch would indicate a bug in one or both encodings.
...
Xilinx Kintex-7 410T (the FPGA family used by OpenTitan
reference implementations). OT-A provides full resources; OT-B restricts LUTs to
...
weaker feature.  Conventional priority ordering would protect crypto
engines first, yet the optimizer correctly exploits redundancy coverage.
This kind of multi-constraint, redundancy-aware assignment requires
exact optimization.
...
\begin{figure}[t]
    ...
    OpenTitan case. Sharp transitions correspond to logging-tier
    and security-tier thresholds; mixed security assignments appear
    under tight budgets.}
    \label{fig:pareto}
\end{figure}
...
\midrule
OT-A & 1{,}804 & 2{,}065 & +14\% & 2{,}065 & +14\% \\
OT-B & 3{,}449 & 4{,}130 & +20\% & 4{,}130 & +20\% \\
OT-C & 1{,}854 & 2{,}445 & +32\% & 2{,}445 & +32\% \\
\bottomrule
\end{tabular}
\end{table}

With latency constraints active, no profile permits a uniform ZT
assignment, so heuristics must make component-level trade-offs from
the start.  The greedy heuristic assigns the lowest-risk
latency-feasible pair to each component in descending impact order,
checking resource caps incrementally.  Local search (1-opt) then
swaps one component at a time to the best feasible neighbour.

Under OT-A, both heuristics find a 2{,}065-risk assignment (14\%
above optimal) because they cannot exploit redundancy-group coupling.
Under OT-C, the gap widens to 32\%: power + latency constraints
together force the optimizer into a redundancy-aware trade (downgrading
crypto accelerators to exploit group coverage) that single-component
swaps cannot reproduce.
...
Since the resource constraints are linear and the objective can be
...
map.  Quantitative what-if analysis at this level requires an exact
solver that reports provably optimal solutions under each constraint set.
...
Cross-validating ASP against CP-SAT on 24 synthetic runs gave us confidence
that the formulation is correct, not just that one solver happens to
find good solutions. The perfect agreement across 24 synthetic runs
was reassuring; the sharp divergence at 20 components was
...
Conventional priority ordering would protect crypto engines first; the
optimizer correctly exploits group structure instead.
