...
def phase1_optimise(strategy: str = "max_security",
                    timeout_s: int | None = DEFAULT_PHASE1_TIMEOUT) -> Phase1Result:
    t0 = time.time()
    ...
    if timeout_s is None:
        solve_desc = "until completion"
    else:
        solve_desc = f"timeout={timeout_s}s"
    log(f"[Phase 1] [{strategy}] Grounding done ({time.time()-t0:.1f}s). "
        f"Solving ({solve_desc})...")
...
def phase3_scenario(sc: dict, p1: Phase1Result, p2: Phase2Result) -> ScenarioResult:
    ...
    for sym in found:
        ...
        elif n == "capability_ok"        and len(a)==1: res.cap_available.append(str(a[0]))
...
def phase3_all(p1: Phase1Result, p2: Phase2Result) -> list:
    ...
    for i, sc in enumerate(SCENARIOS, 1):
        ...
        tag = f"risk={r.total_risk:.1f} resilience={r.resilience_score:.1f}{cp_tag}{cap_tag}" if r.satisfiable else "UNSAT"
...
def generate_report(p1: Phase1Result, p2: Phase2Result,
    ...
    L.append("")
    L.append(f"  {'Scenario':<35} {'Risk':>8} {'Resilience':>11} {'vs Base':>8} {'CP':>5} {'Svcs OK/Deg/Una':>16} {'Caps Lost'}")
    L.append(f"  {'-'*35} {'-'*8} {'-'*11} {'-'*8} {'-'*5} {'-'*16} {'-'*20}")
    ...
    for r in scenarios:
        ...
        L.append(f"  {r.name:<35} {r.total_risk:>8.1f} {r.resilience_score:>11.1f} {ratio:>7.2f}x {cp_flag:>5}  {svc_str:>16} {cap_str}")
