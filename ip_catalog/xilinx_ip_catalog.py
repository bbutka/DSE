...
# ---------------------------------------------------------------------------
# Additive risk model protection values
# ---------------------------------------------------------------------------
# Used by opt_redundancy_generic_enc.lp:
#   Risk = Impact + DomainBonus - SecurityProtect - LogProtect, clamped to 0
# These are cardinal protection scores on the same scale as Impact (1-5).
# zero_trust provides the strongest protection, mac the weakest.

SECURITY_PROTECT_VALS = {
    "zero_trust":   8,   # full packet inspection + policy enforcement
    "dynamic_mac":  6,   # rolling-nonce MAC with FSM
    "mac":          4,   # static MAC authentication token
}

LOG_PROTECT_VALS = {
    "zero_trust_logger": 3,   # deep logging with header + payload digest
    "some_logging":      1,   # basic timestamp/header logging
...
def export_security_features_to_lp(filepath: str | Path) -> Path:
    ...
    for feature in SECURITY_FEATURE_EXPORT_ORDER:
        val = SECURITY_PROTECT_VALS.get(feature, 0)
        lines.append(f"security_protect_val({feature}, {val}).")
    for feature in LOGGING_FEATURE_EXPORT_ORDER:
