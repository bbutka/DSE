...
class Phase1Agent:
    ...
    def _try_relaxed(self, lp_files: List[str], extra: str) -> dict:
        """
        Retry with a relaxed max_asset_risk cap as a fallback.
        Adds an additional permissive fact (500) so the constraint effectively
        uses the higher of the two values — disabling the tight cap.
        This handles cases where the tight cap is infeasible (e.g. redundant
        groups whose probabilistic risk floor exceeds the primary cap).
        """
        relaxed_extra = (
            extra + "\n"
            "% Fallback: add permissive risk cap so either fact satisfies the rule.\n"
            "system_capability(max_asset_risk, 500).\n"
        )
        runner = ClingoRunner(timeout=self.timeout)
        return runner.solve(
            lp_files=lp_files,
